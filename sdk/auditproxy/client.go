...
const defaultUserAgent = "audit-proxy-go-sdk/0.1"
...
// Client is a typed Go SDK client for the Audit Proxy control-plane API.
type Client struct {
	baseURL   string
	token     string
	userAgent string
...
func (e *APIError) Error() string {
	if e.Message != "" {
		return fmt.Sprintf("audit-proxy API error (HTTP %d): %s", e.StatusCode, e.Message)
	}
	if e.Body != "" {
		return fmt.Sprintf("audit-proxy API error (HTTP %d): %s", e.StatusCode, e.Body)
	}
	return fmt.Sprintf("audit-proxy API error (HTTP %d)", e.StatusCode)
}
...
// NewClient creates a new Go SDK client for the Audit Proxy control plane.
