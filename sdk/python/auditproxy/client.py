...
DEFAULT_USER_AGENT = "audit-proxy-python-sdk/0.1"


class AuditProxyError(RuntimeError):
    def __init__(self, status_code: int, message: str = "", body: str = "") -> None:
        self.status_code = status_code
        self.message = message
        self.body = body
        detail = message or body or "unknown error"
        super().__init__(f"audit-proxy API error (HTTP {status_code}): {detail}")
...
class AuditProxyClient:
    ...
    def _request(
        ...
        *,
        ...
        try:
            ...
            raise AuditProxyError(exc.code, payload.get("error", ""), raw) from exc
        except urllib.error.URLError as exc:
            raise AuditProxyError(0, str(exc.reason)) from exc

        payload = self._parse_json(raw)
        if status_code >= 400 or not payload.get("success", False):
            raise AuditProxyError(status_code, payload.get("error", ""), raw)
        return payload
    ...
    def _parse_json(raw: str) -> Dict[str, Any]:
        try:
            ...
            raise AuditProxyError(0, "invalid JSON response", raw) from exc
        if not isinstance(payload, dict):
            raise AuditProxyError(0, "unexpected JSON response", raw)
        return payload
