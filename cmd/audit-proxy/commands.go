...
// runStatus handles "audit-proxy status"
...
// runSessions handles "audit-proxy sessions ..."
func runSessions(args []string) {
	if len(args) == 0 {
		printError("usage: audit-proxy sessions <list|kill> [options]")
		os.Exit(1)
	}
...
func runSessionsKill(args []string) {
	...
	if fs.NArg() == 0 {
		printError("usage: audit-proxy sessions kill <session-id>")
		os.Exit(1)
	}
	sessionID := fs.Arg(0)
...
// runUsers handles "audit-proxy users ..."
func runUsers(args []string) {
	if len(args) == 0 {
		printError("usage: audit-proxy users <list|create|delete|update> [options]")
		os.Exit(1)
	}
...
func runUsersDelete(args []string) {
	...
	if fs.NArg() == 0 {
		printError("usage: audit-proxy users delete <username>")
		os.Exit(1)
	}
	username := fs.Arg(0)
...
func runUsersUpdate(args []string) {
	...
	if fs.NArg() == 0 {
		printError("usage: audit-proxy users update <username> [--display-name NAME] [--role ROLE]")
		os.Exit(1)
	}
	username := fs.Arg(0)
...
// runServers handles "audit-proxy servers ..."
func runServers(args []string) {
	if len(args) == 0 {
		printError("usage: audit-proxy servers <list|add|remove> [options]")
		os.Exit(1)
	}
...
func runServersRemove(args []string) {
	...
	if fs.NArg() == 0 {
		printError("usage: audit-proxy servers remove <server-name>")
		os.Exit(1)
	}
	name := fs.Arg(0)
...
// runAudit handles "audit-proxy audit ..."
func runAudit(args []string) {
	if len(args) == 0 {
		printError("usage: audit-proxy audit search [options]")
		os.Exit(1)
	}
...
// runConfig handles "audit-proxy config ..."
func runConfig(args []string) {
	if len(args) == 0 {
		printError("usage: audit-proxy config <show|edit|reload|history> [options]")
		os.Exit(1)
	}
...
// runCert handles "audit-proxy cert ..."
func runCert(args []string) {
	if len(args) == 0 {
		printError("usage: audit-proxy cert sign [options]")
		os.Exit(1)
	}
...
// runJIT handles "audit-proxy jit ..."
func runJIT(args []string) {
	if len(args) == 0 {
		printError("usage: audit-proxy jit <request|approve|deny|list> [options]")
		os.Exit(1)
	}
...
func runJITApprove(args []string) {
	...
	if fs.NArg() == 0 {
		printError("usage: audit-proxy jit approve <request-id>")
		os.Exit(1)
	}
	requestID := fs.Arg(0)
...
func runJITDeny(args []string) {
	...
	if fs.NArg() == 0 {
		printError("usage: audit-proxy jit deny <request-id>")
		os.Exit(1)
	}
	requestID := fs.Arg(0)
...
// runThreat handles "audit-proxy threat ..."
func runThreat(args []string) {
	if len(args) == 0 {
		printError("usage: audit-proxy threat alerts [options]")
		os.Exit(1)
	}
...
// runCompliance handles "audit-proxy compliance ..."
func runCompliance(args []string) {
	if len(args) == 0 {
		printError("usage: audit-proxy compliance report [options]")
		os.Exit(1)
	}
...
// runVersion handles "audit-proxy version"
