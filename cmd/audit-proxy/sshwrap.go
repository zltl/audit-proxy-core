...
func runWrappedSSH(binary string, args []string, stdin io.Reader, stdout, stderr io.Writer) int {
	...
	if fs.NArg() == 0 {
		fmt.Fprintf(stderr, "error: usage: audit-proxy %s [--addr host:port] <args...>\n", binary)
		return 1
	}
	...
	if targetAddr == "" {
		fmt.Fprintln(stderr, "error: missing SSH proxy address; set --addr, ssh_addr in ~/.audit-proxy/config.json, or AUDITPROXY_SSH_ADDR")
		return 1
	}
