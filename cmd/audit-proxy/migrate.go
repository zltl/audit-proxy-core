...
func printMigrateUsage() {
	fmt.Print(`Usage: audit-proxy migrate <subcommand> [options]

Subcommands:
  ini2db    import users, routes, policies, and IP rules from config.ini

Run "audit-proxy migrate ini2db --help" for the available options.
`)
}

func runMigrateINI2DB(args []string) {
	fs := flag.NewFlagSet("migrate ini2db", flag.ExitOnError)
	configPath := fs.String("config", "/etc/audit-proxy/config.ini", "path to the config.ini to import")
