...
func runPlay(args []string) {
	...
	file := fs.String("file", "", "path to a local asciicast v2 file (skips API download)")
  ...
  audit-proxy play <session-id> [--raw] [--no-timing] [--include-input] [--speed 1.0]
  audit-proxy play --file <path.cast> [--raw] [--no-timing] [--include-input] [--speed 1.0]
...
`)
	...
	case *file != "":
		...
		if err != nil {
			printError(fmt.Sprintf("failed to read recording file: %v", err))
			os.Exit(1)
		}
		body = data
	case fs.NArg() > 0:
	...
	if err := playAsciicast(bytes.NewReader(body), os.Stdout, *speed, !*noTiming, *includeInput); err != nil {
