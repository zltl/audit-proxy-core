// Package main implements the audit-proxy CLI management tool.
//
// Usage: audit-proxy <command> [options]
//
// Commands:
//
...
func printUsage() {
	usage := `Usage: audit-proxy <command> [options]
...
Commands:
  ...
  play        Replay a recorded session (or --file local.cast)
  audit       Query audit logs
  config      Manage configuration
  cert        Manage SSH certificates
...
Run 'audit-proxy <command> --help' for details on a specific command.`
	fmt.Println(usage)
}
