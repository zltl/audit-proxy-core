...
// Client is the HTTP API client for the Audit Proxy control plane.
type Client struct {
	baseURL       string
	token         string
...
// ~/.audit-proxy/config.json and/or environment variables. Env vars take
...
func loadConfig() ClientConfig {
	...
	if env := os.Getenv("AUDITPROXY_SERVER"); env != "" {
		cfg.Server = env
	}
	if env := os.Getenv("AUDITPROXY_TOKEN"); env != "" {
		cfg.Token = env
	}
	if env := os.Getenv("AUDITPROXY_SSH_ADDR"); env != "" {
		cfg.SSHAddr = env
	}
	if env := os.Getenv("AUDITPROXY_IDENTITY_FILE"); env != "" {
		cfg.IdentityFile = env
	}
	if env := os.Getenv("AUDITPROXY_PINNED_SERVER_PUBKEY_SHA256"); env != "" {
...
func configFilePath() (string, error) {
	...
	return filepath.Join(home, ".audit-proxy", "config.json"), nil
}
