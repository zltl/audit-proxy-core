...
func runProxyCommand(args []string) {
	...
	addr := fs.String("addr", "", "SSH proxy TCP address (defaults to config ssh_addr or AUDITPROXY_SSH_ADDR)")
	...
	if targetAddr == "" {
		printError("missing SSH proxy address; set --addr, ssh_addr in ~/.audit-proxy/config.json, or AUDITPROXY_SSH_ADDR")
		os.Exit(1)
	}
